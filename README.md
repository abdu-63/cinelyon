...
L'application mobile de CinéLyon est développée avec **React Native** et **Expo** (SDK 52). Elle se trouve dans le dossier `mobile/`.
...
**Résolution :** 
Ouvrez le fichier [CineLyon.entitlements](mobile/ios/CineLyon/CineLyon.entitlements) et retirez la clé `aps-environment` pour n'avoir qu'un dictionnaire vide :
```xml
